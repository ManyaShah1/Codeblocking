...
import './TutorialsPage.css'; // Create and style this file

// --- XML Samples ---
// I have fixed the syntax errors in your XML below.
const codeSamplesXml = {
  // FIXED: Escaped the backtick (`) with a backslash (\)
  ...
  // FIXED: Logic error in XML (used if/else)
  ...
  // FIXED: Logic error in XML (check num1 % 2)
  ...
  // FIXED: Logic error in XML (join greeting + " " + name)
  ...
  // --- FIXED: Moved this XML string inside the object ---
...
// --- YouTube Video Links ---
// FIXED: Stored only the 'src' URL as a string, not the full iframe tag
const videoLinks = {
...
function TutorialsPage() {
  ...
  const loadSampleIntoWorkspace = (sampleXml) => {
    // Navigate to workspace and pass the XML in the state
    navigate('/workspace', { state: { loadXml: sampleXml } });
  };

  return (
    <div className="tutorials-container page-container"> {/* Added 'page-container' for consistent padding */}
      ...
      <section className="tutorial-section">
        ...
        <ul className="sample-list">
          {Object.entries(codeSamplesXml).map(([key, xml]) => (
            // FIXED: Added onClick to the <li> element for a larger click target
            <li key={key} onClick={() => loadSampleIntoWorkspace(xml)}>
              {/* FIXED: Formatting to replace underscores with spaces */}
              <span>{key.replace(/_/g, ' ').replace(/^./, str => str.toUpperCase())}</span>
              <button className="btn-load-sample">
                Load Sample
              </button>
            </li>
          ))}
          ...
          {Object.entries(videoLinks).map(([key, src]) => (
            <div className="video-wrapper" key={key}>
              <iframe
                ...
                referrerPolicy="strict-origin-when-cross-origin" // Added standard policy
                allowFullScreen>
              </iframe>
            </div>
          ))}
